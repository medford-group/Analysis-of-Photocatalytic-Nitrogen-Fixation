...
\end{figure}
\todoajm{modified caption to explain constant chemical potentials assumed.}

\todobmc{The reviewer raises a good point here. The chemical potential of liquid H2O and 100\% RH is equal by definition (since this is the "equilibrium" vapor pressure). This implies that (d-f) should be a "slice" through (a-c) that intersects (a-c) at the dashed line in (a-c). The "intersection line" in (d-f) should be at pN2 = 0.8 bar. However, getting to a point where "bare surface" is favored over "0.5 ML H2O" requires going much higher in N2 pressure. Something doesn't seem to add up. I suspect that we may have used 1bar H2O in the "aqueous" phase, which is not right. I don't think the conclusions will change, but the diagrams will.}
...
The computational results provide strong evidence that nitrogen oxidation \todobmc{Adjust the oxidative band edge position by 0.2 eV to be consistent with the rutile (rather than anatase) band gap.} \todobmc{revise to the FED's with O* + N2 -> N2O*, add sentence explaining.} \todoajm{The band edges were placed properly in all figures but the redox ladder} is thermodynamically favored on the rutile (110) active site, which is at odds with the experimental observation of reduced products on TiO$_2$\cite{Schrauzer_1977,Schrauzer_1983,Augugliaro_1982,Soria_1991,Schrauzer_2011,Hirakawa_2017} One possible explanation is that nitrogen is first oxidized to NO and subsequently reduced to ammonia. The conversion of NO to NH$_3$ is a 5 electron process with a redox potential at 0.71 V vs. RHE, well below the band gap of TiO$_2$\cite{Medford_2017}. The thermodynamic feasibility of this pathway on rutile (110) has been computed (see Figure \ref{fig:FED_oxid_red}), and the results indicate that this is indeed thermodynamically feasible. Another possible reason that nitrogen oxides are not often observed is that the reverse reaction also takes place on the surface. The reaction of NO to N$_2$O and the reaction of N$_2$O to N$_2$ has been observed under UHV conditions by Yates and colleagues.\cite{Rusu2000,Rusu2001} This may also explain the low activities observed experimentally. However, the complete reverse reaction to N$_2$ is unlikely, due to the instability of N$_2$ on the surface as has been noted in the de-NO$_x$ literature.\cite{} However reverse reactions could, in part, explain why low activity is observed experimentally.\todobmc{find the reference for this} \todobmc{If we have these energies we can add additional FEDs to the SI.}
...
\begin{suppinfo}
\todoajm{This was per the editor comments that there be a summary of everything in the SI. ensure this is all still correct before submitting}
35 pages, 3 tables, 4 figures
Surface energy, coverage, and probability diagram of Rutile TiO$_2$ under reducing conditions\\
Surface energy, coverage, and probability diagram of Rutile TiO$_2$ under oxidizing conditions\\
Comparison of Fe site previously proposed and Fe 2+ site in manuscript\\
Free energy diagram on previously proposed iron site\\
Summary of reactions from paper\\
Table of adsorption energies for all considered species\\
Table of vibrational frequencies for all considered species\\
Table of graphical representations of for all considered species\\
