...
\caption{Redox couples for nitrogen reduction (red) and oxidation (blue) with required driving force ($\Delta$V). The difference between hydrogen evolution and nitrogen reduction ($\mathrm{\Delta \Delta V_{HER}}$) is shown to highlight the selectivity challenge for nitrogen reduction. Band edges for rutile TiO$_2$ were obtained from literature \cite{Nozik_1996}  and are shown (black bar) for reference (note scale break at 1.75 V).}
...
The computational results provide strong evidence that nitrogen oxidation is thermodynamically favored on the rutile (110) active site, which is at odds with the experimental observation of reduced products on TiO$_2$\cite{Schrauzer_1977,Schrauzer_1983,Augugliaro_1982,Soria_1991,Schrauzer_2011,Hirakawa_2017} One possible explanation is that nitrogen is first oxidized to NO and subsequently reduced to ammonia. We have tested the thermodynamic feasibility of this process, the results may be seen in Figure \ref{fig:FED_oxid_red}. The conversion of NO to NH$_3$ is a 5 electron process with a redox potential at 0.71 V vs. RHE, well below the band gap of TiO$_2$\cite{Medford_2017}. Our proposed pathway is quite reasonable from the available data. \todoajm{I added a reference to figure 10 here}

Another possible reason that nitrogen oxides are not often observed could be that the reverse reaction also takes place frequently on the surface. The reaction of NO to N$_2$O and the reaction of N$_2$O to N$_2$ has been observed under UHV conditions by Yates and colleagues.\cite{Rusu2000,Rusu2001} This could help explain the low activities observed experimentally. However, the complete reverse reaction to N$_2$ is quite unlikely, due to the instability of N$_2$ on the surface as has been noted in the de-NO$_x$ literature.\cite{}\todobmc{find the reference for this}
...
This model was also used to assess the effect of photo-excited electrons and holes. The locations of the band edges were obtained from the literature \cite{Nozik_1996} and used to set the potentials of the electrons and holes relative to a zero set by the reversible hydrogen electrode.\todoajm{added a few words to try and address reviewer comment. unsure if this is enough} The potential of holes was set to -0.1V and the potential of excited electrons was set to 2.9V, conserving the experimental value of the band gap at 3 eV \cite{Scanlon_2013}. 
